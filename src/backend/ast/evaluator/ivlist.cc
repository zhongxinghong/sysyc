...
evaluator_ivlist_iterator::evaluator_ivlist_iterator(
  const ASTNode *ast, const std::vector<std::size_t> &dim_list)
  : dim_list(dim_list), pos(0)
{
...
bool evaluator_ivlist_iterator::next()
{
  if (st.empty())
  {
  ...
  val = dynamic_cast<const NumberASTNode *>(ast)->get_val();
  pos++;
  cur.pos++;
